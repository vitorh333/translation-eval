...
# Filtrar apenas o francês
df_fr <- df_long %>% filter(idioma == "fr")

# Criar pasta graphs se não existir
if(!dir.exists("graphs")) dir.create("graphs")

# Caminho do PDF
pdf_file <- "graphs/translation_metrics_fr_v2.pdf"
...
print(
  ggplot(df_fr, aes(x = metrica, y = score, fill = modelo)) +
    geom_bar(
      stat = "identity",
    ...
    labs(
      title = "BLEU e CHRF para Francês",
      x = "",
      y = "Score",
      fill = "Modelo"
    ) +
