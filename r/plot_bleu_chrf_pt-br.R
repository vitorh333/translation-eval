...
# Filtrar apenas o português
df_pt <- df_long %>% filter(idioma == "pt-BR")

# Criar pasta graphs se não existir
if(!dir.exists("graphs")) dir.create("graphs")

# Caminho do PDF
pdf_file <- "graphs/translation_metrics_pt-BR_v2.pdf"
# Abrir dispositivo PDF
pdf(pdf_file, width = 10, height = 5)

# Gerar e imprimir gráfico
print(
  ggplot(df_pt, aes(x = metrica, y = score, fill = modelo)) +
    geom_bar(
      stat = "identity",
    ...
    labs(
      title = "BLEU e CHRF para Portugues (Brasil)",
      x = "",
      y = "Score",
      fill = "Modelo"
    ) +
