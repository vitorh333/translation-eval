...
# Filtrar apenas o espanhol (México)
df_es_mx <- df_long %>% filter(idioma == "es-MX")

# Criar pasta graphs se não existir
if(!dir.exists("graphs")) dir.create("graphs")

# Caminho do PDF
pdf_file <- "graphs/translation_metrics_es-MX.pdf"
...
print(
  ggplot(df_es_mx, aes(x = metrica, y = score, fill = modelo)) +
    geom_bar(
      stat = "identity",
    ...
    labs(
      title = "BLEU e CHRF para Espanhol (Mexico)",
      x = "",
      y = "Score",
      fill = "Modelo"
    ) +
